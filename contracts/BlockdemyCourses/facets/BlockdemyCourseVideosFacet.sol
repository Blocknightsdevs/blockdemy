...
contract BlockdemyCourseVideosFacet is Modifiers{
    ...
    ) public IsOwner(_courseId) {
        require(_uris.length==_titles.length,'Uris and Titles Not Same Length');
        for (uint256 i = 0; i < _uris.length; i++) {
            s._courseUris[_courseId].push(_uris[i]);
            s._videoTitles[_courseId].push(_titles[i]);
        }
    }
    ...
    function editCourseVideos(
        ...
        require(_uris.length==_title.length,'Uris and Titles Not Same Length');
        for (uint256 i = 0; i < _uris.length; i++) {
            s._courseUris[_courseId].push(_uris[i]);
